...
static void printnum_padding(void (*putch)(int, void*), void *putdat,
	 unsigned long long num, unsigned base, int width, int padc)
{
	...
	while(num_tmp>0)
	{
		num_rev[num_len_tmp]=num_tmp%base;//calculate each bit of the num

		num_tmp=num_tmp/base;
		num_len_tmp++;
	}

	int num_len = num_len_tmp;

	while(num_len_tmp>0)
	{
		putch("0123456789abcdef"[num_rev[num_len_tmp-1]], putdat);
		num_len_tmp-=1;
	}

	int width_padding = width - num_len;
	 ...
	 unsigned long long num, unsigned base, int width, int padc)
{
	...
	if(padc == '-')
	{
		printnum_padding(putch, putdat, num , base, width , padc);
	}
	else
	{
		if (num >= base) {
...
vprintfmt(void (*putch)(int, void*), void *putdat, const char *fmt, va_list ap)
{
	...
	int base, lflag, width, precision, altflag, signedflag;
	char padc;
	...
	while (1) {
		...
		lflag = 0;
		altflag = 0;
		signedflag = 0;//show the sign or not
	reswitch:
		switch (ch = *(unsigned char *) fmt++) {

		// flag to pad on the right
		case '+'://if has detected '+',turn on the sign signal 
			signedflag = 1;
			goto reswitch;
		case '-':
			padc = '-';
		...
		case 'd':{
			...
			else if((long long) num > 0 && signedflag ==1)
			{
				putch('+', putdat);
				num = (long long) num;
			}
		...
		case 'n': {
				  ...
				  // Your code here
				  char * n_p;
				  if ((n_p = va_arg(ap, char *)) == NULL){
					  printfmt(putch, putdat, "%s", null_error); //dealing null error
				  }
				  else if( (*((int *)putdat))<0 ||(*((int *)putdat))>240){
					  *n_p = -1;
					  printfmt(putch, putdat, "%s", overflow_error); //dealing overflow error  
				  }
				  else{
					  *n_p = *(char *)putdat;
				  }
