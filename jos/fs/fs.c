...
file_block_walk(struct File *f, uint32_t filebno, uint32_t **ppdiskbno, bool alloc)
{
	...
	// Set '*ppdiskbno' to point to that slot. The slot will be one of the f->f_direct[] entries
	else if(filebno < NDIRECT){
		*ppdiskbno = &(f->f_direct[filebno]);
	}
	// Set '*ppdiskbno' to point to that slot. The slot will be an entry in the indirect block.
	else{
		if(!f->f_indirect){
...
file_get_block(struct File *f, uint32_t filebno, char **blk)
{
	...
	if(*pdiskbno == 0){
		*pdiskbno = alloc_block();
		// if a block needed to be allocated but the disk is full.
		if(*pdiskbno < 0){
			return -E_NO_DISK;
		}
	}
