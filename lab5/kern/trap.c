...
trap_init(void)
{
	...
	SETGATE(idt[T_FPERR], 0, GD_KT, fpu_floating_point_error, 0);
	SETGATE(idt[T_ALIGN], 0, GD_KT, alignment_check, 0);
	SETGATE(idt[T_MCHK], 0, GD_KT, machine_check, 0);
	SETGATE(idt[T_SIMDERR], 0, GD_KT, floating_point_exception, 0);
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 3) == 0) 
	{
    	panic("tarp->page_fault_handler: kernel-mode page faults \n");
	}
	...
	if(curenv->env_pgfault_upcall != NULL){
		// Set up a page fault stack frame on the user exception stack (below UXSTACKTOP)
		...
		// Store the fault_va into utf
		utf->utf_fault_va = fault_va;
		// Store the current data of tf into utf
		...
		// use env_run() to branch to curenv->env_pgfault_upcall
