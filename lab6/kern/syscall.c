...
sys_env_set_status(envid_t envid, int status)
{
	...
	// LAB 4: Your code here.
	// if status is not a valid status for an environment
	...
	struct Env *e;
	if(envid2env(envid,&e,1)<0){// set envid2env's third argument to 1
		return -E_BAD_ENV;
	}
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	// Set envid's trap frame to 'tf'.
	e->env_tf = *tf;
	e->env_tf.tf_cs = GD_UT | 3; // always run at code protection level 3 (CPL 3)
	e->env_tf.tf_eflags |= FL_IF; // interrupts enabled
	e->env_tf.tf_eflags &= FL_IOPL_0; // and IOPL of 0.
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	// modifying the corresponding struct Env's 'env_pgfault_upcall' field
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// if there's no memory to allocate the new page, or to allocate any necessary page tables.
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// if srcva is not mapped in srcenvid's address space.
	pte_t *src_pte;
	struct PageInfo *pi = page_lookup(src_env->env_pgdir, srcva, &src_pte);
	if (pi ==  NULL){
		return -E_INVAL;
	}
	...
	if (page_insert(dst_env->env_pgdir, pi, dstva, perm) < 0){
		return -E_NO_MEM;
	}
...
{
    // LAB3: your code here.
	void* va_start = ROUNDDOWN((void*)curenv->env_break - inc, PGSIZE);
	void* va_end = ROUNDUP(va_start + inc, PGSIZE);
	void* va_tmp = va_start;
	
	int time = (va_end - va_start)/PGSIZE;
	int i = 0;
	while(i < time)
	{
		struct PageInfo* nPage = page_alloc(ALLOC_ZERO);
		if(nPage == NULL)
		{
			panic("syscall->sys_sbrk: page alloc failed!");
		}
		page_insert(curenv->env_pgdir,nPage,va_tmp,PTE_U|PTE_W);
		va_tmp += PGSIZE;
		i++;
	}

	curenv->env_break = (uintptr_t)ROUNDDOWN(curenv->env_break-inc, PGSIZE);
...
sys_net_send(const void *buf, uint32_t len)
{
	...
	int ret_chk = user_mem_check(curenv, buf, len, PTE_U | PTE_P);
	if(ret_chk < 0){
		return ret_chk;
	}
	// Call e1000_tx to send the packet
	int ret = e1000_tx(buf, len);
...
sys_net_recv(void *buf, uint32_t len)
{
	// LAB 6: Your code here.
	// Check the user permission to [buf, buf + len]
	int ret_chk = user_mem_check(curenv, buf, len, PTE_U | PTE_P | PTE_W);
	if (ret_chk < 0){
		return ret_chk;
	}
	// Call e1000_rx to fill the buffer
	int ret = e1000_rx(buf, len);
