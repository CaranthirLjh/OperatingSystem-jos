...
send_data(struct http_request *req, int fd)
{
	...
	while (true) 
	{
		...
		ret = read(fd, file_buf, BUFFSIZE);
		if (ret < 0) {
			cprintf("send_data: read-> %e\n", ret);
			return ret;
		}

		int len = ret;
		// Send the data
		if ((ret = write(req->sock, file_buf, len)) != len) {
			cprintf("send_data: read-> %d\n", ret);
		}
		// send_data finished ?
		if (len < BUFFSIZE)
		{
